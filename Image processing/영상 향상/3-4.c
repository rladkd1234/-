...
void bmpBWrw()
{
	...
	*/
	...
	for (int i = 11; i < (H - 10); i++)
	for (int j = 11; j < (W - 10); j++)
		temp[i][j] = window * (integralImg[i + 10][j + 10] + integralImg[i-11][j-11] - integralImg[i + 10][j-11] - integralImg[i-11][j +10]);
	...
	*/
	...
	// Image Output
	sprintf(outfilename, "3-4 %s", filename);
