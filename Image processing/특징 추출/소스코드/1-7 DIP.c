...
void bmpBWrw()
{
	...
	BYTE *DIP = malloc(hInfo.biSizeImage);
	memset(DIP, 0, hInfo.biSizeImage);
	...
	for (int j = 2; j < rwsize - 3; j++){
		...
		if (Img[i*rwsize + j] == 0)
			...
			result = ((double)(sum / 25.) / (Img[i*rwsize + j])) - ((double)(sum / 25.) / max);
	...
	for (int j = 1; j < rwsize - 2; j++) {
		...
		//printf("temp = %f\n", temp[i*rwsize + j]);
		DIP[i * rwsize + j] = (BYTE)(temp[i * rwsize + j] * 255. + 0.5);
	...
	sprintf(outfilename, "1-7) DIP %s", filename);
	FILE *outfile = fopen(outfilename, "wb");
	fwrite(&hf, sizeof(char), sizeof(BITMAPFILEHEADER), outfile);
	fwrite(&hInfo, sizeof(char), sizeof(BITMAPINFOHEADER), outfile);
	fwrite(hRGB, sizeof(RGBQUAD), 256, outfile);
	fwrite(DIP, sizeof(char), hInfo.biSizeImage, outfile);
	fclose(outfile);
