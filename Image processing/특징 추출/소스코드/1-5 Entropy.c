...
void bmpBWrw()
{
	...
	BYTE *Ep = malloc(hInfo.biSizeImage * sizeof(double));
	memset(Ep, 0, hInfo.biSizeImage);
	double p[5][5] = { 0, };
	...
	for (int j = 2; j < rwsize - 3; j++){
		...
		for (int k = 0; k < 5; k++){
			for (int e = 0; e < 5; e++)
			{
				sum += Img[(i - 2 + k) * rwsize + j - 2 + e];
				p[k][e] = (double)Img[(i - 2 + k) * rwsize + j - 2 + e];
		...
		for (int k = 0; k < 5; k++){
			for (int e = 0; e < 5; e++)
			{
				...
				//fsum += p[k][e];
				//Entropy
				temp[i*rwsize + j] -= (double)(p[k][e]* (log10(p[k][e]) / log10(2.)));
				//printf("logb10(p[k][e]) = %f\n", log10(p[k][e]));
			}
		}
		
		result = temp[i*rwsize + j];
		if (result > Emax)
			Emax = result;
	...
	//	printf("temp = %f\n", temp[i*rwsize + j]);
		Ep[i * rwsize + j] = (BYTE)(temp[i * rwsize + j] * 255. +0.5);
	...
	sprintf(outfilename, "1-5) Entropy %s", filename);
	FILE *outfile = fopen(outfilename, "wb");
	fwrite(&hf, sizeof(char), sizeof(BITMAPFILEHEADER), outfile);
	fwrite(&hInfo, sizeof(char), sizeof(BITMAPINFOHEADER), outfile);
	fwrite(hRGB, sizeof(RGBQUAD), 256, outfile);
	fwrite(Ep, sizeof(char), hInfo.biSizeImage, outfile);
	fclose(outfile);
